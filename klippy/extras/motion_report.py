...
class DumpTrapQ:
    ...
    def get_trapq_position(self, print_time):
        ...
        return pos, velocity, move.accel
...
class PrinterMotionReport:
    ...
    def _dump_shutdown(self, eventtime):
        ...
        if dtrapq is None:
            return
        pos, velocity, accel = dtrapq.get_trapq_position(shutdown_time)
        if pos is not None:
            logging.info(
    ...
    def get_status(self, eventtime):
        ...
        pos, velocity, accel = self.trapqs["toolhead"].get_trapq_position(print_time)
        if pos is not None:
            xyzpos = pos[:3]
        ...
        if ehandler is not None:
            pos, velocity, accel = ehandler.get_trapq_position(print_time)
            if pos is not None:
                epos = (pos[0],)
                evelocity = velocity
        # Report status
        ...
        self.last_status["live_acceleration"] = xyzaccel
        return self.last_status
